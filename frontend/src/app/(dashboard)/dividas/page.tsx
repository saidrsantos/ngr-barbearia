...
function InstallmentsList({ debtId }: { debtId: number }) {
  ...
  if (loading) return <p className="px-4 py-3 text-xs text-gray-400 dark:text-gray-500">Carregando parcelas...</p>;

  return (
    <div className="divide-y divide-gray-100 border-t border-gray-100 bg-gray-50 dark:divide-gray-800 dark:border-gray-800 dark:bg-gray-800/40">
      {installments.map((inst) => (
        <div key={inst.id} className="flex items-center justify-between px-4 py-2 text-sm">
          <div>
            <span className="font-medium text-gray-900 dark:text-gray-100">Parcela {inst.installment_number}</span>{' '}
            <span className="text-gray-500 dark:text-gray-400">
              — vence {new Date(inst.due_date + 'T12:00:00').toLocaleDateString('pt-BR')}
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-gray-700 dark:text-gray-300">{formatBRL(inst.amount_cents)}</span>
            <button
              onClick={() => toggle(inst)}
...
export default function DividasPage() {
  ...
  return (
    <div>
      <h1 className="mb-1 text-lg font-semibold text-gray-900 dark:text-gray-100">Dívidas dos barbeiros</h1>
      <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
        Compras/adiantamentos que você faz pro barbeiro (ex: máquina de corte), pagos de volta em parcelas — pra não
        perder conta na hora do fechamento.
      </p>
      ...
      >
        <select required value={barberId} onChange={(e) => setBarberId(e.target.value)} className={inputClass}>
        ...
        <button
          type="submit"
          disabled={saving}
          className="self-end rounded-md bg-emerald-700 px-3 py-2 text-sm font-medium text-white hover:bg-emerald-600 disabled:opacity-50 dark:bg-emerald-600 dark:hover:bg-emerald-500 sm:col-span-3"
        >
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {debts.map((debt) => (
            <div key={debt.id} className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
              <button
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  {debt.pending_count > 0 ? (
                    <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-medium text-amber-800 dark:bg-amber-500/10 dark:text-amber-400">
          ...
          {debts.length === 0 && <p className="text-sm text-gray-400 dark:text-gray-500">Nenhuma dívida registrada ainda.</p>}
        </div>
      )}
    </div>
  );
